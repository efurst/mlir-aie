...
def print_config_json(pid_events):
    # print("pid_events:"+str(len(pid_events)))
    type0_size = len(pid_events[0])
    type1_size = len(pid_events[1])
    # print("type0_size:"+str(type0_size))
    # print("type1_size:"+str(type1_size))
    # print("pid_event[0].items:"+str(type(pid_events[0].items)))
    try:
        with open("config.json", "wt") as f:
            ...
            # TODO iterate over pid_events[0] for core tile, 
            # pid_events[1] for core mem tile
            for idx, (key, value) in enumerate(pid_events[0].items()):
                loc = key
                eventArray = value
                f.write("          {\n")
                ...
                f.write("            }\n")
                f.write("          }\n")
                # print("JL idx: "+str(idx))
                if(idx < type0_size-1):
                    f.write("            ,\n")
            f.write("        ],\n")
            # TODO iterate over pid_events[3] for memtiles
            f.write('        "MemTileTraceConfig": [],\n')
            # TODO iterate over pid_events[2] for interface/shim tile
...
# hwfrontend --trace prep.trace.txt --trace_config config.json --pkg-dir . --outfile eventIR.txt
